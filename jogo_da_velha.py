...
def verificaGanhador(board):
    # verifica se uma das 3 linhas estão preenchidas e retorna o token em caso de sucesso
    for i in range(3):
        if board[i][0] == board[i][1] and board[i][1] == board[i][2] and board[i][0] != branco:
            return board[i][0]

    # verifica se uma das 3 colunas estão preenchidas e retorna o token em caso de sucesso
    for i in range(3):
